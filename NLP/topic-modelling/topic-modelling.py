...
def save_topics (list_lda_topics, list_hdp_topics, category_id):
    ...
    topics_file = open (topics_file_path % category_id, "w") # Create file object (w = write)
...
# Function to update the TopicID of the FeedbackML dataframe
def update_feedback_ml_topic_id (series):
    
    # Check length of TextTopics
    if (len (series ['TextTopics']) > 0):

        # Update the TopicID of the current Feedback-Topic mapping
        series ['TextTopics'] = [dict_topic_id_mapping [int (item)] for item in series ['TextTopics']]
...
# File paths
train_file_path = "%sdata/feedback-ml" % working_directory + "-%s.csv"                  # Dataset file path (prior to topic modelling)
topic_file_path = '%sdata/feedback-ml-topics' % working_directory + "-%s.csv"           # Topic modelled dataset file path
topics_file_path = '%sdata/topics' % working_directory + "-%s.txt"                      # File path of topic details
topics_df_file_path = '%sdata/topics' % working_directory + "-%s.csv"                   # File path of topics table
feedback_topics_df_file_path = '%sdata/feedback-topics' % working_directory + "-%s.csv" # File path of feedback-topics table
manual_tagging_file_path = '%sdata/manual-tagging.txt' % working_directory              # Manually tagged topic-tokens file path
pickles_file_path = "%spickles/" % working_directory                                    # File path containing pickled objects
...
for optimal_model in list_models:
    ...
    if (topic_model_data == True):
        ...
        feedback_ml_df.to_csv (train_file_path % category_id, index = False, encoding = "utf-8")
        ...
        if (use_manual_tag == True): # Implement manual tagging 
            ...
            if (len (list_topics_assigned) > 0):
                ...
                # Update the value of the largest TopicID
                largest_topicid = get_largest_topicid () + 1
        ...
        # Check length of Topics DataFrame
        if (len (topic_df) > 0):
        ...
        # pyLDAvis.save_html (lda_visualise, accuracy_file_path + 'lda-%s.html' % category_id) # Export visualisation to HTML file

        # Save models (pickling/serialization)
        pickle_object (lda_model, "lda-model-%s.pkl" % category_id) # LDA Model
        pickle_object (hdp_model, "hdp-model-%s.pkl" % category_id) # HDP Model
        pickle_object (id2word, "id2word-%s.pkl" % category_id)     # Dictionary of ID-Word mappings for each category

        # Export and save DataFrames
        feedback_ml_df.to_csv (topic_file_path % category_id, index = False, encoding = "utf-8") # Save FeedbackML DataFrame
        topic_df.to_csv (topics_df_file_path % category_id, index = False, encoding = "utf-8") # Save Topics DataFrame
        feedback_topic_df.to_csv (feedback_topics_df_file_path % category_id, index = False, encoding = "utf-8") # Save FeedbackTopic DataFrame
